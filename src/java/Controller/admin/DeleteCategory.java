 ...
 * @author vuatr
 */
@WebServlet(name = "DeleteCategory", urlPatterns = {"/delete"})
public class DeleteCategory extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean isDeleted = pd.deleteCategory(id);

            if (isDeleted) {
                // Xóa thành công
                response.sendRedirect("listCategory");
            } else {
                // Không thể xóa do có sách liên kết
                request.setAttribute("message", "Không xóa được vì đã có sách trong kho.");
